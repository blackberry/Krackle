...
public class ProducerConfiguration {
	...
	if (requestRequiredAcks != -1 && requestRequiredAcks != 0
	    ...
	    throw new Exception(
		    "request.required.acks can only be -1, 0 or 1.  Got "
			    + requestRequiredAcks);
	}
	LOG.info("request.required.acks = {}", requestRequiredAcks);

	requestTimeoutMs = Integer.parseInt(props.getProperty(
	...
	if (messageSendMaxRetries < 0) {
	    throw new Exception(
		    "message.send.max.retries cannot be negative.  Got "
	...
	if (messageBufferSize < 1) {
	    throw new Exception(
		    "message.buffer.size must be greater than 0.  Got "
			    + messageBufferSize);
	}
	...
	if (responseBufferSize < 1) {
	    throw new Exception(
		    "response.buffer.size must be greater than 0.  Got "
			    + responseBufferSize);
	}
	LOG.info("response.buffer.size = {}", responseBufferSize);

	String rawCompressionCodec = props.getProperty("compression.codec",
		"none");
