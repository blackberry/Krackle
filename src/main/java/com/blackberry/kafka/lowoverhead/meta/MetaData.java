...
public class MetaData {
	...
	List<String> metadataBrokerList = Arrays
		.asList(metadataBrokerListString.split(","));
	...
	try {
		...
		int read = in
			.read(responseArray, bytesRead, length - bytesRead);
		if (read == -1) {
		    throw new IOException(
			    "Stream ended before end of response.");
		}
		bytesRead += read;
	    }
		...
		metadata.getBrokers().put(nodeId,
			new Broker(nodeId, host, port));
		...
		for (int j = 0; j < numParts; j++) {
		    ...
		    LOG.debug("    Partition ID={}, Leader={} (Error={})",
			    leader, partId, partError);
