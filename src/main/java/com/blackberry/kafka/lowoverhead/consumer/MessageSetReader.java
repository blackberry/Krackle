...
public class MessageSetReader {
    ...
    public int getMessage(byte[] dest, int pos, int maxLength)
	    throws IOException {
	if (messageSetReader != null && messageSetReader.isReady()) {
		...
		if (bytesCopied < valueLength) {
		    LOG.warn("Truncating message from {} to {} bytes.",
			    valueLength, maxLength);
		}
		System.arraycopy(bytes, buffer.position(), dest, pos,
			bytesCopied);
		...
		if (messageSetReader.isReady()) {
		    bytesCopied = messageSetReader.getMessage(dest, pos,
			    maxLength);
		}
	...
	while (true) {
	    decompressedSize = decompressor.decompress(bytes,
		    buffer.position(), valueLength, decompressionBytes, 0,
		    decompressionBytes.length);
