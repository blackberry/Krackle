...
public class LowOverheadConsumer {
	...
	messages = this.metrics.meter(MetricRegistry.name(
		LowOverheadConsumer.class, "messages retrieved [" + topic + "-"
			+ partition + "]"));
	bytes = this.metrics.meter(MetricRegistry.name(
		LowOverheadConsumer.class, "bytes retrieved [" + topic + "-"
			+ partition + "]"));
	...
	while (bytesReturned == -1) {
	    while (messageSetReader == null
		    || messageSetReader.isReady() == false) {
		readFromBroker();
	    }
	...
	try {
	    ...
	    // then the
	    // partition id (4 bytes)
	    responseBuffer.position(responseBuffer.position() + 4 + 2
		    + topicLength + 4 + 4);
	...
	while (true) {
		...
		MetaData meta = MetaData.getMetaData(
			conf.getMetadataBrokerList(), topic, clientId);
